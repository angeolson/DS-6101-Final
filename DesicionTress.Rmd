...
xkabledply(treefit1$cptable)
tree_prune1 <- prune(treefit1, cp = 0.01)
fancyRpartPlot(tree_prune1)

predict_model1<-predict(tree_prune1,test)
#confusionMatrix(test$new_genres, predict_model1)
...
treefit2<-rpart(music_genre ~ popularity + acousticness + danceability + energy + key +liveness + loudness + mode + speechiness + valence, data=training)
fancyRpartPlot(treefit2)

xkabledply(treefit2$cptable)
tree_prune2 <- prune(treefit2, cp = 0.01)
fancyRpartPlot(tree_prune2)

predict_model2<-predict(tree_prune2, test)
```
