...
```{r, results = 'markup'}
loadPkg("rpart")
music_tree<-subset(music, music_genre == 6)
str(music_tree)
set.seed(1)
treefit <- rpart(music_genre ~ liveness + loudness, data=music_tree, method="class", control = list(maxdepth = 14) )
loadPkg("rpart.plot") 
tree <- rpart.plot(treefit)
summary(tree)
loadPkg("rattle")
