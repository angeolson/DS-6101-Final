...
treefit2<-rpart(new_genres ~ popularity + acousticness + danceability + energy + key +liveness + loudness + mode + speechiness + valence, data=new, method="class", control = list(maxdepth = 4))
fancyRpartPlot(treefit2)
printcp(treefit2)

cm = confusionMatrix( predict(treefit2, type = "class"), reference = new$new_genres )
```
...
k10=knn(train = training[,c("liveness", "energy")], test = testing[,c("liveness", "energy")], cl=training[,"new_genres"], k=10)
#summary(k10)
