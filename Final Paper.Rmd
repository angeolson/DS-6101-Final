...
First, we look at how many genresow many genres there are and how many observations are in each genre. In the table below, we see that we are lookng at `r nrow(genres)` genres, each with roughly 4,500 obersvations, making this a balanced dataset.  
...
First, we build a multinomial Logit model with the following variables:
...
However, when using a model calibrated to predict classical or not, "La Donna è Mobile," is predicted to be classicial--but barely, at a probability of `r predict (Model_Class, test[ test$instance_id == '38780', ], type = 'response')`. 
